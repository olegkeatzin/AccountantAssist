...
def summarize_with_ollama(
    ...
    system_prompt = """Ты - эксперт по техническим товарам, строительным материалам и промышленному оборудованию. 
...
Твоя задача - создавать краткие, информативные и понятные описания товаров. 
...
Задание: На основе информации выше создай краткое описание товара (2-4 предложения). Укажи что это такое, где применяется и для чего используется. Используй конкретные факты из текста.

Описание:"""
    else:
        # Если информации нет - попробуем создать описание на основе названия
        prompt = f"""Товар: {product_name}{comment_context}

Задание: Создай краткое описание этого товара (2-4 предложения). Проанализируй название, объясни что это, где применяется и для чего используется. Используй технические термины если они есть в названии.
...
Описание:"""
    ...
    try:
        ...
        if any(phrase.lower() in description.lower() for phrase in negative_phrases) or len(description) < 20:
            ...
            simple_prompt = f"""Товар: {product_name}

Создай краткое техническое описание этого товара (2-3 предложения). Опиши что это, где применяется и для чего используется.{comment_context if comment else ""}
