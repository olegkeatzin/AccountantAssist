...
def summarize_with_ollama(
    ...
    if search_results:
        ...
        prompt = f"""Ты - эксперт по техническим товарам и материалам. Твоя задача - создать краткое и понятное описание товара на основе предоставленной информации.
...
ВАЖНО: 
- Создай краткое описание товара (2-4 предложения)
- Укажи основное назначение и применение
- Используй простой и понятный язык
- НЕ пиши "я не смог найти информацию" или подобные фразы
- Если информации мало, опирайся на название и контекст

Описание товара:"""
    else:
        # Если информации нет - попробуем создать описание на основе названия
        prompt = f"""Ты - эксперт по техническим товарам и материалам. Твоя задача - создать краткое описание товара на основе его названия.

Название товара: "{product_name}"{comment_context}

ВАЖНО:
- Проанализируй название товара и разбери его по частям
- Создай краткое описание (2-4 предложения) о назначении и применении
- Используй профессиональную терминологию, но объясняй простым языком
- НЕ пиши "я не знаю" или "не могу найти информацию"
- Опирайся на свои знания о похожих товарах и материалах
...
Описание товара:"""
    ...
    try:
        ...
        # Если описание содержит негативные фразы, попробуем еще раз с более высокой температурой
        if any(phrase.lower() in description.lower() for phrase in negative_phrases):
            logger.warning(f"  - ⚠️ Модель дала негативный ответ, повторная попытка...")
            response = client.generate(
                model=model,
                prompt=prompt + "\n\nОбязательно предоставь описание на основе названия товара, используй свои знания.",
                options={
                    'temperature': 0.7,
                    'num_predict': 250,
                    'top_p': 0.9
